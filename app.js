...
async function triggerBackgroundSync() {
  ...
  try {
    ...
    showToast(`Syncing ${queue.length} offline transaction(s)...`, 'warning');
    ...
    for (const item of queue) {
      ...
      try {
        ...
        failedCount++;
        console.warn(`Sync fetch failed for item ID ${item.id}. Keeping in queue:`, payload, err);
      }
    }
    ...
    if (syncedCount > 0 && failedCount === 0) {
      ...
      showToast(`Synced ${syncedCount} transaction(s). ${failedCount} still pending.`, 'warning');
    } else if (failedCount > 0) {
      showToast(`${failedCount} transaction(s) could not sync. Will retry when online.`, 'warning');
    }
...
async function populateItemSelects() {
  ...
  try {
    const items = await window.StockDB.getAllItems();
    select.innerHTML = '<option value="">-- Choose Hardware SKU --</option>' + 
